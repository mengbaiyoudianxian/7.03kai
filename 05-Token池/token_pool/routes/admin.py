...
<div class="page" id="page-users">
  <div class="panel"><div class="panel-header"><h2>乌托邦token池</h2><button class="btn-outline" onclick="probeAllUsers()" id="btn-probe-users">一键检测全部</button></div>
  <div class="panel-body"><table><thead><tr><th>设备码</th><th>Provider / Model</th><th>昨日用量</th><th>共享比例</th><th>今日配额</th><th>已借出</th><th>状态</th><th>操作</th></tr></thead><tbody id="users-tbody"></tbody></table></div></div>
</div>
...
function freeKeyAction(code,action){
  var u="/api/free-keys/"+encodeURIComponent(code)+(action==="refresh"?"/refresh":"/adjust");
  var b=action==="refresh"?null:JSON.stringify({status:action==="enable"?"active":"disabled"});
  api(u,{method:"POST",body:b}).then(function(){loadFreeKeys()}).catch(function(e){toast(""+e.message,1)})
}
function freeKeyAdjust(code,field,val){
  var body={};body[field]=field==="status"?val:parseInt(val)||0;
  api("/api/free-keys/"+encodeURIComponent(code)+"/adjust",{method:"POST",body:JSON.stringify(body)}).then(function(){toast("已更新")}).catch(function(e){toast(""+e.message,1)})
}
async function loadFreeKeys(){var t=$("free-tbody");spin(t);try{var d=await api("/api/free-keys");$("free-stats").innerHTML="<div class=\"stat-card\"><div class=\"label\">总数</div><div class=\"value\">"+d.length+"</div></div><div class=\"stat-card\"><div class=\"label\">活跃</div><div class=\"value\" style=\"color:var(--green)\">"+d.filter(function(x){return x.status==="active"}).length+"</div></div><div class=\"stat-card\"><div class=\"label\">总消耗</div><div class=\"value\">"+d.reduce(function(s,x){return s+(x.used_total||0)},0).toLocaleString()+"</div></div><div class=\"stat-card\"><div class=\"label\">今日消耗</div><div class=\"value\">"+d.reduce(function(s,x){return s+(x.used_today||0)},0).toLocaleString()+"</div></div>";if(!d.length){none(t,"暂无");return}t.innerHTML=d.map(function(k){var up=k.total_limit>0?((k.used_total/k.total_limit)*100).toFixed(0):0,dp=k.daily_limit>0?((k.used_today/k.daily_limit)*100).toFixed(0):0,st=k.status==="active"?"<span class=\"status status-ok\">活跃</span>":"<span class=\"status status-err\">禁用</span>";return"<tr><td style=\"font-family:var(--mono);font-size:12px\">"+k.code+"</td><td style=\"font-size:12px;color:var(--text-tertiary)\">"+(k.device_code||"").slice(0,16)+"</td><td style=\"font-size:12px\">"+k.ip_address+"</td><td><input type=\"number\" value=\""+k.total_limit+"\" onchange=\"freeKeyAdjust('"+k.code+"','total_limit',this.value)\" style=\"width:120px;height:26px;font-size:11px\"> <span style=\"font-size:11px\">"+k.used_total.toLocaleString()+"</span></td><td><input type=\"number\" value=\""+k.daily_limit+"\" onchange=\"freeKeyAdjust('"+k.code+"','daily_limit',this.value)\" style=\"width:120px;height:26px;font-size:11px\"> <span style=\"font-size:11px\">"+k.used_today.toLocaleString()+"</span></td><td><input type=\"number\" value=\""+k.rpm_limit+"\" onchange=\"freeKeyAdjust('"+k.code+"','rpm_limit',this.value)\" style=\"width:55px;height:26px;font-size:11px\"></td><td>"+st+"</td><td><button type=\"button\" class=\"btn-sm\" onclick=\"freeKeyAction('"+k.code+"','refresh')\">重置</button><button type=\"button\" class=\"btn-sm\" onclick=\"freeKeyAction('"+k.code+"','"+(k.status==="active"?"disable":"enable")+"')\">"+(k.status==="active"?"禁用":"启用")+"</button></td></tr>"}).join("")}catch(e){toast(""+e.message,1);none(t,"加载失败")}}
