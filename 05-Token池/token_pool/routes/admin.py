...
function renderKeys(){var q=($("filter-keys")&&$("filter-keys").value||"").toLowerCase(),ks=q?_keys.filter(function(k){return(k.alias+k.provider+k.model).toLowerCase().indexOf(q)>=0}):_keys,t=$("keys-tbody");if(!ks.length){none(t,_keys.length?"无匹配":"暂无 Key");return}t.innerHTML=ks.map(function(k){var b=k.circuit_open?"<span class=\"status status-cb\">熔断</span>":k.status==="working"?"<span class=\"status status-ok\">正常</span>":k.status==="failed"?"<span class=\"status status-err\">失败</span>":"<span class=\"status status-warn\">未知</span>",m=k.has_key?"":" <span style=\"color:var(--amber-text);font-size:11px\">缺Key</span>",r=k.circuit_open?"<button class=\"btn-sm\" style=\"color:var(--green)\" onclick=\"api('/api/keys/'+encodeURIComponent('"+k.alias+"')+'/reset_circuit',{method:'POST'}).then(function(){loadKeys()})\">恢复</button>":"";return"<tr><td><strong>"+k.alias+"</strong>"+m+"</td><td>"+k.provider+"</td><td style=\"color:var(--text-tertiary)\">"+k.model+"</td><td>"+b+"</td><td>"+k.success_count+"/<span style=\"color:var(--red)\">"+k.fail_count+"</span></td><td>"+(k.avg_latency_ms?k.avg_latency_ms.toFixed(0)+"ms":"-")+"</td><td>$"+(k.total_cost||0).toFixed(5)+"</td><td>"+k.priority+"</td><td style=\"white-space:nowrap\">"+r+"<button class=\"btn-sm\" onclick=\"editKey('"+k.alias+"')\">编辑</button><button class=\"btn-sm danger\" onclick=\"if(confirm('确认删除?'))api('/api/keys/'+encodeURIComponent('"+k.alias+"'),{method:'DELETE'}).then(function(){loadKeys()})\">删除</button></td></tr>"}).join("")}
